...
const LocationFilterIcon = () => (
    <svg 
        width="14" // Adjust desired width
        height="14" // Adjust desired height
        viewBox="0 0 24 24" 
        fill="none" 
        xmlns="http://www.w3.org/2000/svg" 
        className="inline-block align-middle text-gray-500" // Use Tailwind text color, inline-block, and align-middle
    >
        <path fillRule="evenodd" clipRule="evenodd" d="M11.9455 3.00158C11.9639 3.00158 11.9819 3.00079 11.9999 3C12.0179 3.00079 12.0358 3.00158 12.0543 3.00158C16.4358 3.06442 20 6.36 20 10.3362C20 15.6 14.2751 19.2338 11.9999 21C11.8479 20.8819 4 15.6 4 10.3362C4 6.36 7.5644 3.06458 11.9455 3.00158Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
        <path fillRule="evenodd" clipRule="evenodd" d="M12 13C13.3807 13 14.5 11.8807 14.5 10.5C14.5 9.11929 13.3807 8 12 8C10.6193 8 9.5 9.11929 9.5 10.5C9.5 11.8807 10.6193 13 12 13Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
);
...
const currentUser = { initial: 'S' };
const BurgerIconFull = () => ( <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16"></path></svg> ); // Changed burger icon for better visibility
...
const DashboardPage = () => {
    ...
    const fetchProfilesData = useCallback(async () => {
        ...
        try {
            ...
            console.log("Fetched Profiles Data:", data); // Keep this for debugging
            setProfiles(data || []);
    ...
    return (
        <div className="min-h-screen">
            <div className={`max-w-screen-2xl-plus mx-auto flex min-h-screen lg:shadow-xl lg:my-0 max-lg:w-full max-lg:mx-0 max-lg:shadow-none`}>
                <aside className={`w-64 bg-sidebar-bg flex-shrink-0 flex flex-col border-r border-gray-200 print:hidden md:w-60 ${isMobileSidebarOpen ? 'max-md:flex max-md:fixed max-md:inset-y-0 max-md:left-0 max-md:z-30 max-md:w-64 max-md:shadow-xl' : 'max-md:hidden'}`}>
                    <div className="flex flex-col flex-grow p-4 pt-6">
                        ...
                        <div className="mt-auto pt-4">
                            ...
                            <div className="px-1">
                                <button onClick={handleLogout} className="w-full bg-auth-primary hover:bg-auth-primary-hover text-white py-2.5 px-3 rounded-md text-sm font-medium">Logout</button>
                            </div>
                        </div>
                    </div>
                    {isMobileSidebarOpen && <button onClick={() => setIsMobileSidebarOpen(false)} className="absolute top-4 right-4 text-gray-500 hover:text-gray-700 md:hidden"><CloseIconFull /></button>}
                </aside>
                
                {isMobileSidebarOpen && <div onClick={() => setIsMobileSidebarOpen(false)} className="fixed inset-0 bg-black opacity-50 z-20 md:hidden"></div>}

                <div className="flex-grow flex flex-col bg-white overflow-y-auto">
                    <header className="md:hidden sticky top-0 z-10 bg-sidebar-bg p-4 border-b border-gray-200 flex items-center justify-between print:hidden">
                        ...
                        <div className="w-8 h-8 rounded-avatar bg-brand-avatar-bg text-white flex items-center justify-center text-sm font-semibold">{currentUser.initial}</div>
                    </header>

                    <div className={`p-6 md:p-8 ${isMobileSidebarOpen ? 'max-md:blur-sm' : ''}`}>
                        ...
                        <section className="flex flex-col md:flex-row gap-4 mb-7 items-center"> {/* Added items-center */}
                            <input type="text" placeholder="Enter Role (e.g., Developer)" className="p-2.5 h-10 border border-gray-300 rounded-md flex-1 text-sm focus:ring-1 focus:ring-brand-focus-ring focus:border-brand-focus-ring" value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} />
                            ...
                            <div className="relative flex-1"> {/* Location input with custom icon */}
                                
                                <select value={locationFilter} onChange={(e) => setLocationFilter(e.target.value)} className="p-2.5 h-10 border border-gray-300 rounded-md w-full text-sm focus:ring-1 focus:ring-brand-focus-ring focus:border-brand-focus-ring appearance-none pr-8"> {/* Added pl-10 for icon space */}
                                    <option value="">Select Location</option>
                                    {manualLocationOptions.map(loc => (<option key={loc} value={loc}>{loc}</option>))}
                                </select>
                                <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none"><DropdownArrowIcon /></div>
                            </div>
                            <button className="p-2 h-10 border border-gray-300 rounded-md flex-shrink-0 hover:bg-gray-50" title="More filters"> {/* Added hover effect */}
                                <GeneralFilterIcon />
                            </button>
                        </section>
                        ...
                        {/* PROFILE GRID - Ensuring it can display many cards */}
                        {!loading && !error && profiles.length > 0 && (
                            <section className="grid grid-cols-2 gap-4 p-4 print:grid-cols-2 print:gap-2"> {/* md:grid-cols-2 is key */}
                                {profiles.map((profile) => (
                                    <article key={profile._id} className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm flex flex-col hover:shadow-md transition-shadow">
                                        <div className="flex items-center mb-4">
                                            ...
                                            <button className="ml-3 bg-brand-button hover:opacity-90 text-white py-1.5 px-3.5 rounded-md text-xs font-medium flex-shrink-0">View Profile</button>
                                        </div>
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {profile.skills.map((skill, idx) => (
                                                        <div key={idx} className="bg-gray-100 border border-gray-200 text-gray-700 px-2.5 py-1 rounded-md text-xs flex flex-col items-start">
                                                            ...
                                                            <span className="text-[10px] text-gray-500 ml-[calc(14px+0.25rem+0.25rem)]"> {/* 14px icon width + mr-1 + small indent */}
